/** @file
 * @brief Implements NCEP bug fix. Replaces certain NCEP-defined time ranges by the 
 * WMO equivalents.
...
/**
 * "Set" option that replaces certain NCEP-defined time ranges by the WMO equivalents. 
 * 
 * The current wgrib2 needs this option in order to correctly print out the time ranges 
 * for NCEP-defined time ranges. 
 * 
 * The -fix_ncep option changes the memory image of the grib message. Therefore, if you 
 * want the NCEP time range, you must have the -grib or -grib_out option before the 
 * -fix_ncep option. Otherwise you will get the WMO time range. 
 * 
 ...
 * @endcode
 * 
 * The above line replaces NCEP time ranges by the WMO equivalents and writes out the 
 * new grib data into out_grib.
 *
