/** @file
 * @brief Put vector fields into same submessage - for multiprocessing.
 *
 * Based on NCEP_uv.c
 *
 * To alter the U/V list, use -new_grid_vectors. U and V must be adjacent.
 * ### Program History Log
 * Date | Programmer | Comments
 * -----|------------|---------
 * 1/2015 | W. Ebisuzaki | Initial
 * 10/2019 | W. Ebisuzaki | Added fflush_file()
 * @author Public Domain: Wesley Ebisuzaki @date 1/2015
 */
...
/** Pointer to grid vectors. */
extern const char **vectors;

/**
 * Check if the given name is a U component and return the corresponding V component name.
 *
 * @param uname Pointer to the U component name.
 *
 * @return Pointer to the corresponding V component name or NULL if not found.
 *
 * @author Wesley Ebisuzaki @date 1/2015
 */
...
/**
 * Combines U and V vector fields into a single submessage.
 * 
 * The option -submsg_uv is like the option -ncep_uv. The latter option puts UGRD and VGRD into 
 * the same grib message. The former combines all the vector fields into their own grib message. 
 * The option -submsg_uv may be important for parallel processing. For example, you want to 
 * regrid from a Gaussian grid to a polar stereographic grid that covers the North Pole. For 
 * scalar fields, the grid can be regridded independently which allows a possibility of parallel 
 * processing. Vector fields have to processed together (U and V). One approach is to put the 
 * vector fields into their own grib message (combine the corresponding U/V fields). Then all the 
 * grib messages can be regridded independently. 
 * 
 * The vector fields are the same as used by the -new_grid option. The vector list can be changed 
 * using the -new_grid_vectors option. 
 * 
 * ## Usage
 * -submsg_uv FILE
 * 
