/** @file
 * @brief Duplicate NCEP files by putting U and V into one GRIB message.
 *
 * U and V must be adjacent.
 * @author Public Domain: Wesley Ebisuzaki @date 9/2010
 */
...
/**
 * Combine U and V fields into one message like NCEP operations.
 *
 * Operational NCEP grib files often have the U and corresponding V fields in the same grib 
 * message. (U and V are submessages.) Typically wgrib2 converts all the submessages into 
 * individual messages. In order to combine the U and V fields together, you replace the 
 * -grib option with -ncep_uv. Options like -grib_out and -new_grid do not have the capability 
 * to combine the U and V fields. For theses cases, you run the output through wgrib2. Note 
 * that -ncep_uv is more like -grib than -grib_out. The option -ncep_uv uses the compressed 
 * grid point values from the original file. Neither the grid point values nor the packing 
 * is changed.
 *
 * ## Usage
 * -ncep_uv FILE
 ...
 *
 * @author Wesley Ebisuzaki @date 9/2010
 */
int f_ncep_uv(ARG1) {
