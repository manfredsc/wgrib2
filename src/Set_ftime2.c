/** @file
 * @brief Routine to set forecast time in GRIB sections.
 *
 * Based on Public Domain codes -set_ftime and -set_ave. ftime2 is the replacement of the 
 * original ftime (fixes problems).
 * 
 * Ftime2.c       grib -> text version of ftime
 * Set_ftime2.c   text version of ftime -> grib
 * @author Public Domain: Wesley Ebisuzaki @date 4/2016
 */
...
/** Struct for statistical processing. */
struct stat_proc {
	...
	int code_4_4b;				/**< Time units for successive fields */
	int d_timeb;				/**< Length of time for successive fields */
};
...
/** Maximum number of time range specifications. */
#define N_MAX 10

/** Struct for statistical time processing. */
struct stat_time {
   struct full_date ref_time;		    /**< Reference time. */
   struct full_date verf_time;		    /**< End of overall processing time. */
   int code_4_4_fcst;                   /**< Forecast time units. */
   int d_time_fcst;                      /**< Length of forecast time. */
   ...
   struct stat_proc stat_procs[N_MAX];  /**< Array of stat proc specifications. */
};
...
/**
 * Set forecast time in GRIB sections. (same as -set_ftime).
 * 
 ...
 * 
 * Wgrib2 has options to change the forecast time (ftime). The -set_ftime2 option is easier
 * to use and will change the Product Definition Templates as needed. For compatibility, -set_ftime1 
 * is still available. However, -set_ftime1 should not be used as will be eventually removed. 
 * 
 * This routine is also called by the -set_ftime option.
 * 
 * ## Usage
 * -set_ftime2 FTIME
 * 
 ...
 * -set_ftime FTIME
 * 
 * The allowed formats of FTIME are the same as ftimes from a wgrib2 inventory.
 * 
 ...
 * 
 * @author Wesley Ebisuzaki @date 4/2016
 */
...
/**
 * Parses the time range from a string and removes ",missing=N" suffix.
 *
 * @param arg Pointer to string containing the time range.
 * @param new_stat_time Pointer to struct to fill with parsed time range information.
 * @param n Should be zero.
 *
...
/**
 * Recursively parses the time range from a string and fills in new_stat_time->stat_procs[n];
 * 
 * The statistical processing format is nested, stat_procs[0] is the outer definition
 *
 * @param arg Pointer to string containing the time range.
 * @param new_stat_time Pointer to struct with stat_procs array.
 * @param n Size of the stat_procs array. Should be < N_MAX.
 *
 * @return 0 for success, 1 for failure.
 */
